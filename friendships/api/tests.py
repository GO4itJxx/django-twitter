...
class FriendshipApiTests(TestCase):
    ...
    def test_followers_pagination(self):
        ...
        for i in range(page_size * 2):
            follower = self.create_user('jingxi_follower{}'.format(i))
            Friendship.objects.create(from_user=follower, to_user=self.jingxi)
            if follower.id % 2 == 0:
                Friendship.objects.create(from_user=self.jingxi, to_user=follower)

        url = FOLLOWERS_URL.format(self.jingxi.id)
        ...
        # jiemin has followed users with even id
        response = self.jiemin_client.get(url, {'page': 1})
        for result in response.data['results']:
    ...
    def test_followings_pagination(self):
        ...
        page_size = FriendshipPagination.page_size
        for i in range(page_size * 2):
            following = self.create_user('jingxi_following{}'.format(i))
            Friendship.objects.create(from_user=self.jingxi, to_user=following)
            if following.id % 2 == 0:
                Friendship.objects.create(from_user=self.jiemin, to_user=following)

        url = FOLLOWINGS_URL.format(self.jingxi.id)
        self._test_friendship_pagination(url, page_size, max_page_size)
        ...
        # I have followed all my following users
        response = self.jingxi_client.get(url, {'page': 1})
        for result in response.data['results']:
